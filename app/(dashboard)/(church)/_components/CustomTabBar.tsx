...
export function CustomTabBar({
  ...
  const tabConfig: Record<
    string,
    { icon: keyof typeof Ionicons.glyphMap; label: string; order: number }
  > = useMemo(
    () => ({
      home: { icon: "home", label: t("navbar.home"), order: 1 },
      members: { icon: "people", label: t("navbar.members"), order: 2 },
      teams: { icon: "grid", label: t("navbar.teams"), order: 3 },
      settings: { icon: "ellipsis-horizontal", label: t("navbar.more"), order: 4 },
    }),
    [t]
  );

  const sortedRoutes = [...state.routes].sort((a, b) => {
    const routeNameA = a.name.split("/")[0];
    const routeNameB = b.name.split("/")[0];
    const orderA = tabConfig[routeNameA]?.order || 999;
    const orderB = tabConfig[routeNameB]?.order || 999;
              ...
              {isFocused ? (
                ...
                <Animated.View
                  style={{
                    ...
                    paddingHorizontal: 8,
                    opacity:
                      scaleAnims[route.key]?.interpolate({
                        inputRange: [0.9, 1],
                        outputRange: [0.6, 1],
                        extrapolate: "clamp",
                      }) || 1,
